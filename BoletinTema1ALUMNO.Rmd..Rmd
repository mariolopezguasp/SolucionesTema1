...
## PARA GENERAR SOLO LOS ENUNCIADO 
include=FALSE
...
## PARA Incluir la soluciones SOLO LOS ENUNCIADO 
include=TRUE
...
```{r}
mam <- rownames(mammals)
ani <- rownames(Animals2)
commonAnimals <- intersect(mam, Animals)
commonAnimals
...
noani <- setdiff(mam,ani)

nomam <- setdiff(mam, ani)

noaninomam <- union(noani, nomam)
noaninomam
...
```{r}
dif1 <- setdiff(noani, nomam)

dif2 <- setdiff(nomam, noani)
...
```{r}
log_brain_mammals <- log(mammals$brain)
log_brain_animals2 <- log(Animals2$brain)

# Gráfico qqPlot para mammals
qqPlot(log_brain_mammals, main = "mammals")

# Gráfico qqPlot para Animals2
qqPlot(log_brain_animals2, main = "Animals2")
...
sum(is.na(cabbages$Weight))
sum(is.na(cabbages$Head))
sum(is.na(cabbages$Leaf))

head(Chile)
summary(Chile)
str(Chile)

sum(is.na(Chile$income))
sum(is.na(Chile$savings))
sum(is.na(Chile$expenditure))
...
```{r}
qqPlot(cabbages$HeadWt, main = "qqPlot for HeadWt")
qqPlot(cabbages$VitC, main = "qqPlot for VitC")
```
...
```{r}
hist(cabbages$VitC[cabbages$Cult], 
       main = "Distribución VitC, Calabaza")
```
